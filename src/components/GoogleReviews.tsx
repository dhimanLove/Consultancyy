...
export function GoogleReviewsBadge({ className = "" }: { className?: string }) {
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-[13px] font-semibold text-navy shadow-sm transition-all hover:shadow-md hover:border-navy/20 active:scale-[0.98] ${className}`}
    >
      <span className="text-[12px] font-bold text-[#4285F4]">Google</span>
      <span className="flex items-center gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star key={i} className="w-3.5 h-3.5 text-[#FBBC05] fill-current" />
        ))}
      </span>
      <span className="font-bold text-navy">{data.rating}</span>
      {data.totalReviews > 0 && <span className="text-steel">({data.totalReviews})</span>}
